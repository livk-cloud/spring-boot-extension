...
class StorageAutoConfigurationTests {
	...
	static {
		try (DynamicType.Unloaded<Object> unloaded = new GenericsByteBuddy().subclass(Object.class)
			.name("com.livk.storage.marker.StorageMarker")
			.make()) {
	...
	final ApplicationContextRunner contextRunner = new ApplicationContextRunner().withUserConfiguration(Config.class)
		...
		.withPropertyValues("spring.storage.endpoint=http://localhost:9000", "spring.storage.access-key=admin",
				"spring.storage.secret-key=1375632510", "spring.storage.type=minio");
