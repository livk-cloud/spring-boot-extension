...
package com.livk.context.elasticsearch.template;
...
import com.livk.context.elasticsearch.annotation.Document;
...
public class ElasticsearchTemplate {
	...
	public void deleteIndex(List<String> names) throws IOException {
		if (!exist(names)) {
			log.info("索引：{} -> 删除索引失败，索引不存在", StringUtils.collectionToDelimitedString(names,
					org.apache.commons.collections4.CollectionUtils.COMMA));
			return;
		}
		...
		if (acknowledged) {
			log.info("索引：{} -> 删除索引成功", StringUtils.collectionToDelimitedString(names,
					org.apache.commons.collections4.CollectionUtils.COMMA));
		}
		else {
			log.info("索引：{} -> 删除索引失败", StringUtils.collectionToDelimitedString(names,
					org.apache.commons.collections4.CollectionUtils.COMMA));
		}
	}
	...
	public <T> void createDocument(String index, String id, T obj) throws IOException {
		...
		if (StringUtils.hasLength(response.result().jsonValue())) {
	...
	public <T> CompletableFuture<Void> asyncCreateDocument(String index, String id, T obj) {
		return elasticsearchAsyncClient.index(idx -> idx.index(index).refresh(Refresh.True).id(id).document(obj))
			.thenAcceptAsync(resp -> {
				if (StringUtils.hasLength(resp.result().jsonValue())) {
	...
	public <T> void bulkCreateDocument(String index, Map<String, T> map) {
		try {
			...
			log.error("批量同步索引失败，错误信息：{}", ex.getMessage());
		}
	}
	...
	public boolean exist(List<String> names) throws IOException {
		try {
			...
			log.error("索引：{} -> 索引是否存在失败，错误信息：{}", StringUtils.collectionToDelimitedString(names,
					org.apache.commons.collections4.CollectionUtils.COMMA), ex.getMessage());
			return false;
		}
	}
	...
	private <R> void setId(R source, String id) {
		try {
			...
			log.error("ID赋值失败，错误信息：{}", ex.getMessage());
		}
	}
	...
	private List<NamedValue<HighlightField>> getHighlightFields(Set<Search.HighlightField> fields) {
		return fields.stream()
			.map(item -> NamedValue.of(item.getName(), new HighlightField.Builder()
	...
	private TokenFilter getFilter(List<Document.Option> options) {
		...
		filterBuilder.definition(fn -> fn.withJson(new ByteArrayInputStream(JsonMapperUtils.writeValueAsBytes(map))));
	...
	private <TDocument> Document convert(String name, String alias, Class<TDocument> clazz) {
		...
		if (annotationPresent) {
			...
			alias = StringUtils.hasLength(alias) ? alias : name;
	...
	private <TDocument> List<Document.Mapping> getMappings(Class<TDocument> clazz) {
		...
		return Arrays.stream(fields)
			.filter(item -> item.isAnnotationPresent(com.livk.context.elasticsearch.annotation.Field.class))
			.map(item -> getMapping(item, item.getAnnotation(com.livk.context.elasticsearch.annotation.Field.class)))
			.toList();
	}

	private Document.Mapping getMapping(Field item, com.livk.context.elasticsearch.annotation.Field field) {
		String value = field.value();
		value = !StringUtils.hasLength(value) ? item.getName() : value;
