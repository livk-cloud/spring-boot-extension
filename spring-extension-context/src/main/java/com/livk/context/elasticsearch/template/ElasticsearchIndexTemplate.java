...
import com.livk.context.elasticsearch.annotation.SubField;
...
public final class ElasticsearchIndexTemplate {
	...
	public void deleteIndex(List<String> names) throws IOException {
		...
		printLog(StringUtils.collectionToDelimitedString(names, "、"),
		...
		return asyncExist(names, executor).thenApplyAsync(resp -> {
			...
			if (result) {
				elasticsearchAsyncClient.indices()
					...
					.thenAcceptAsync(response -> printLog(StringUtils.collectionToDelimitedString(names, "、"),
	...
	public boolean exist(List<String> names) {
		try {
			...
			log.error("index name: 【{}】 -> Failed to determine if the index exists, error message: {}",
					StringUtils.collectionToDelimitedString(names, "、"), ex.getMessage(), ex);
			return false;
		}
	}
	...
	private <TDocument> Document getDocument(String name, String alias, Class<TDocument> clazz) {
		...
		if (annotationPresent) {
			...
			alias = StringUtils.hasText(alias) ? alias : name;
	...
	private Document.Mapping getMapping(Field item, com.livk.context.elasticsearch.annotation.Field field) {
		...
		value = !StringUtils.hasText(value) ? item.getName() : value;
