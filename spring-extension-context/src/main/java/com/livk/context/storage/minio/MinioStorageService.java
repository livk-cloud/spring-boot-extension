...
package com.livk.context.storage.minio;
 ...
 */
@Slf4j
public class MinioStorageService extends AbstractStorageService<MinioClient> {

	@Setter
	private String policyTemplate = "{" + "  \"Statement\": [" + "    {" + "      \"Action\": \"s3:GetObject\","
	...
	public void createBucket(String bucketName) {
		if (!this.exist(bucketName)) {
			...
			// 设置桶策略
			String config = policyTemplate.replaceAll("\\$\\{bucketName}", bucketName);
	...
	@SneakyThrows
	@Override
	public void removeObjs(String bucketName, String... fileNames) {
		if (!ObjectUtils.isEmpty(fileNames)) {
			List<DeleteObject> list = Arrays.stream(fileNames).map(DeleteObject::new).toList();
			Iterable<Result<DeleteError>> results = client
				.removeObjects(RemoveObjectsArgs.builder().bucket(bucketName).objects(list).build());
