...
class ProtobufPlugin : Plugin<Project> {
    ...
    private fun createCompileProtoPathConfiguration(protoSourceSet: ProtoSourceSet): Configuration {
        ...
        return project.configurations.create(compileProtoConfigName) {
            it.isVisible = false
            it.isTransitive = true
            it.extendsFrom(compileClasspathConfig)
            it.isCanBeConsumed = false
            it.attributes {
                it.attribute(
                    LibraryElements.LIBRARY_ELEMENTS_ATTRIBUTE,
                    project.getObjects().named(LibraryElements::class.java, LibraryElements.RESOURCES)
                )
                    .attribute(
    ...
    private fun addTasksForSourceSet(
        ...
        val closure: () -> Unit = {
            project.plugins.withId("eclipse") {
                generateProtoTask.get().getOutputSourceDirectories().all { outputDir ->
                    outputDir.mkdirs()
                }
            }
            ...
            project.plugins.withId("idea") {
                ...
                generateProtoTask.get().getOutputSourceDirectories().forEach { outputDir ->
