...
public class BatchConfig {
	...
	public ItemWriter<User> writer(DataSource dataSource) {
		...
		writer.setSql(
				"insert into sys_user(user_name, sex, age, address, status, create_time) values (:userName, :sex, :age, :address, :status, :createTime)");
		return writer;
	}

	@Bean
	public Step csvStep(StepBuilderFactory stepBuilderFactory, DataSource dataSource) {
		return stepBuilderFactory.get("csvStep").<User, User>chunk(5).reader(reader()).processor(processor())
				.writer(writer(dataSource)).faultTolerant()
				...
				.noSkip(FileNotFoundException.class).build();
	}

	@Bean
	public Job csvJob(JobBuilderFactory jobBuilderFactory, Step step, JobCompletionListener listener) {
		return jobBuilderFactory.get("csvJob").start(step).listener(listener).build();
	}
